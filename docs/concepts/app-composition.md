...
surface = (
    ...
    .mount("diagram", diagram_app, click={"render_options": render_options})
    .mount("diagram.project", project_app, mcp={"page_size": 10})
)
...
explicitly scoped to their projection, so transport settings never leak between adapters. Duplicate paths and a path that
